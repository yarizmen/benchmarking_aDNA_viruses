...
viral_sequences_lgth <- ggplot() +
  ...
  scale_fill_manual(values = c("RS" = "#E69F00", "v120" = "#0072B2"), 
                    labels = c("RS" = "10,544 RefSeq viral sequences", 
                               "v120" = "120 selected viral sequences"),
                    name = "Legend") +
...
ggsave("~/Dropbox/Virome/PLoS_ONE_virome/figures_2021_03_19/figure_1_viral_sequences_lgth.pdf",
       viral_sequences_lgth, width = 297, height = 210, scale = 0.75, units = "mm")
```
...
create_df_condition <- function(path2data, condition, condition_values, decimals = F) {
  ...
  if (decimals == T) {
    condition_values <- paste0("0.", condition_values) %>% as.numeric(.)
  }
...
# Deaminaion variation

```{r length variation data}
deamination_variation <- create_df_condition(path2data = sim_data, 
                                             condition = "Deamination/Deam_singlstr_",
                                             condition_values = c(0, 05, 1, 15, 2, 25, 3, 35, 4, 
                                                                  45, 5),
                                             decimals = T)
```

```{r plots on grid}
deamination_plots <- comparison_ggplot_lines(condition_df = deamination_variation, 
                                        classifier_colors = classifier_colors,
                                        ylabs = c(rep("%", 6), "# identified", 
                                                  "# extra taxa reported"), 
                                        xlabs = rep("deamination in single strand parts", 8), 
                                        titles = c("Proportion classified", 
                                                   "Proportion correct species", 
                                                   "Proportion correct higher",
                                                   "Proportion incorrect classified",
...
g_deaminations <- grid.arrange(grobs = deamination_plots, ncol = 2, nrow = 4)
ggsave("~/Downloads/Deamination.pdf", g_deaminations, width = 7, height = 10, units = "in", 
       scale = 2)
```
