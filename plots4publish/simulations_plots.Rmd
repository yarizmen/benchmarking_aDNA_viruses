...
This part corresponds to create the figure regarding the sequence length of the 120 viral sequences used in the study.
...
Histogram of RefSeq with overlapping step plot of the 120 viral sequences. Final figure regarding length.
...
This section is to create the figures regarding the first simulation. The first simulation was using simulated reads of 60 bp long, without damage, and with the default sequencing error.
...
```{r Centrifuge 60bp}
# Classified reads
Centrifuge_120v_results <- fread(file = paste0(Centrifuge_path, "Centrifuge_classified.txt"))
setnames(Centrifuge_120v_results, "V1", "class")
# Unclassified reads
Centrifuge_120v_results[, unclass := (total_sim_reads - Centrifuge_120v_results$V1)]
# Correct classified: species level
Centrifuge_120v_results[, Correct_Sp := fread(file = paste0(Centrifuge_path,
                                                            "Correct/", "Centrifuge_correct"),
                                              header = F, select = c(2))]
# Correct classified: higher taxonomic level
Centrifuge_120v_results[, Correct_HT := 
                          fread(file = paste0(Centrifuge_path, 
                                              "Higher_taxa/", "Centrifuge_HigTaxa"), 
                                header = F, select = c(2))]
# Incorrect classified reads
Centrifuge_120v_results[, Incorrect := (class - (Correct_Sp + Correct_HT))]
```

```{r DIAMOND 60bp}
# Classified & unclassified reads
DIAMOND_120v_results <- transpose(fread(file = paste0(DIAMOND_path, "DIAMOND_ClassUnc.txt")))
names(DIAMOND_120v_results) <- c("class", "unclass")
# Correct classified: species level
DIAMOND_120v_results[, Correct_Sp := fread(file = paste0(DIAMOND_path,
                                                            "Correct/", "DIAMOND_correct"),
                                              header = F, select = c(2))]
# Correct classified: higher taxonomic level
DIAMOND_120v_results[, Correct_HT := 
                          fread(file = paste0(DIAMOND_path, 
                                              "Higher_taxa/", "DIAMOND_HigTaxa"), 
                                header = F, select = c(2))]
# Incorrect classified reads
DIAMOND_120v_results[, Incorrect := (class - (Correct_Sp + Correct_HT))]
```
...
classifiers_names <- c(rep("Centrifuge", 4), rep("Kraken2", 4), rep("DIAMOND", 4),
                       rep("MetaPhlAn2", 4)) %>% factor(., levels = c("Centrifuge", "Kraken2",
                                                                      "DIAMOND", "MetaPhlAn2"))
category <- c(rep(c("Correct species", "Correct higher", "Incorrect", "Unclassified"), 4)) %>% factor(., levels = c("Correct species", "Correct higher", "Incorrect", "Unclassified"))
...
Violin plot only for Centrifuge data to show that there is nothing wrong, but violin plots won't help us this time.
...
This section is to create a barplot as before, but instead the overall proportion using the median of the data.

Function to get the overall proportions (with or without median). And function to create data frames for ggplot2. 
...
Grouped barplot by classifier for the median of the proportions. Overlapping jittered points for each virus observation.
