---
title: "Figure 09 10 human sims taxRanks "
...
r <- ggplot(d4, aes(x = rank2, y = N)) +
  ...
  coord_flip() 
r
ggsave(paste0(folder,"/figure_09_human_sims_taxRanks.pdf"), width = 297, height = 210*2/3, scale=1, units = "mm")

```

## plot data with most hits
```{r}
d <- dataHuman[dataHuman$classification == "Incorrect"  & dataHuman$taxID != 1,]

## get the rank
nodes <- fread(paste0(folder,"/","nodes.dmp"), sep="|")
nodes$V1 <- as.character(nodes$V1)
d <- merge(d, nodes[,c(1,3)], by.x='taxID', by.y='V1', all.x=T)
colnames(d)[5] <- 'rank'
d$rank <- trimws(d$rank)

## get the taxName
names <- fread(paste0(folder,"/","names.dmp"), sep="|")
names <- names[names$V4 == "\tscientific name\t",]
names$V1 <- as.character(names$V1)
d <- merge(d, names[,c(1,2)], by.x='taxID', by.y='V1', all.x=T)
colnames(d)[6] <- 'taxName'
d$taxName <- trimws(d$taxName)
...
ggsave(paste0(folder,"/figure_10_human_sims_abudantTaxa.pdf"), width = 297, height = 210*2/3, scale=1.5, units = "mm")


## write Excel sheet with all hits
d1 <- d[,c('Classifier','reads','taxID','taxName','rank'),]
library("xlsx")
file <- paste0(folder,"/table_S03_human_simulation_extra_taxa.xlsx")
for(i in classifiers){
  write.xlsx(d1[d1$Classifier == i, -1, with = F], file, sheetName = i, 
